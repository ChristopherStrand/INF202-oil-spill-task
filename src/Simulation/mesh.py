...
class Mesh:
    ...
    def __init__(self, msh_file: str, cell_factory: CellFactory) -> None:
        ...
        self._points = [
            cls.Point(index, np.float32(points[0]), np.float32(points[1]))
    ...
    def cells_within_area(
        self, x_area: npt.NDArray[np.float32], y_area: npt.NDArray[np.float32]
    ) -> list[cls.Cell]:
        cells_within = []
    ...
    def _find_neighbors(self, cell: cls.Cell) -> list[cls.Cell]:
        ...
        neighboring_cells = [
            ...
            if len(points_in_cell & set(cells.points)) == 2
        ]
    ...
    def calculate_change(self, cell: cls.Cell, dt: float):
        ...
        def _g(
            a: float, b: float, v: npt.NDArray[np.float32], w: npt.NDArray[np.float32]
        ):
            """
        ...
        for index, neighbor in enumerate(neighbors):
            ...
            flux = flux + (
                -(dt / cell.area)
                * _g(cell.oil_amount, neighbor.oil_amount, scaled_normal, v_mid)
            )
